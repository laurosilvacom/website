...
import {sendResendEmail} from '@/shared/integrations/resend'
...
function getDeliveryKey(workshopSlug: string, email: string, lessonKey: string) {
...
export async function processWorkshopDripQueue(limit = 50) {
	...
	for (const deliveryKey of readyKeys.slice(0, limit)) {
		const delivery = (await redis.get(deliveryKey)) as WorkshopDripDelivery | null
		if (!delivery) {
