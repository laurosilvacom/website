...
function cached<T>(fetcher: () => Promise<T>, key: string[], tags: string[]): Promise<T> {
...
export async function fetchModuleBySlug(slug: string): Promise<Workshop | null> {
	return cached(
		async () => {
			const workshopData = await client.fetch<Workshop | null>(moduleBySlugQuery, {
				slug,
			})
			return workshopData
		},
